...
const numToString = (num) => {
    ...
    if (num < 10 || num > 99)
        return `число вне диапазона`; // если число вне диапазона 10 - 99
    if (num < 20)
        return array11[units];
    else
...
function getLongString() {
    ...
    for (let i = 0; i < arguments.length; i++)
        result.push(arguments[i]); // проходим по аргументам и собираем их значения в массив
...
function calc(example = '42+58') {
    let numbers = example.match(/\d+/gi); // только числа
    let operator = example.match(/\D/gi); // все кроме чисел
...
const strSeparator = (str = '10/08/2020', sep = '/') => {
    const result = []; // создаем массив для хранения результата
    let ar = 0;
    result[0] = '';
